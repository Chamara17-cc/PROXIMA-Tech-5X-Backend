        ...
        public async Task<ActionResult> GetProjectsDetails(int clientId)
        {
            ...
            foreach (var project in myProjects)
            {
                ...
                var projectDto = new DashboardProjectDto
                {
                    ...
                    Total = await _dataContext.Budgets.Where(e => e.ProjectId == project.ProjectId).Select(b => b.TotalCost).FirstOrDefaultAsync(), // Use null conditional operator to handle null case
                    TotalTask = totalTask,
                    

                    CompletedTask = completedTask,
                    MonthlyPayment=(int)await _dataContext.Budgets.Where(e => e.ProjectId == project.ProjectId).Select(b => b.TotalCost).FirstOrDefaultAsync()/ months,
